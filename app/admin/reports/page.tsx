...
const mockRevenue = [1200, 2800, 1800, 3400, 2900, 4100, 3750];
const mockOrders = [34, 72, 51, 98, 82, 120, 105];
...
export default function ReportsPage() {
  const totalRevenue = mockRevenue.reduce((s, v) => s + v, 0);
  const totalOrders = mockOrders.reduce((s, v) => s + v, 0);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatsCard icon={<DollarSign className="text-orange-500" />} accent="bg-orange-50" label="Total Revenue" value={`$${(totalRevenue/1000).toFixed(1)}K`} trend={+14} />
        <StatsCard icon={<ShoppingCart className="text-emerald-500" />} accent="bg-emerald-50" label="Total Orders" value={totalOrders} trend={+8} />
        <StatsCard icon={<Users className="text-sky-500" />} accent="bg-sky-50" label="New Customers" value={89} trend={+5} />
        <StatsCard icon={<TrendingUp className="text-indigo-500" />} accent="bg-indigo-50" label="Conversion Rate" value="6.8%" trend={+1} />
      </div>
